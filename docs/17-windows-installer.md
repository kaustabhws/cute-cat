...
**1.1.1 update:** first normal app launch registers current-user Windows startup by default, as explicitly requested. Later opt-outs persist; uninstall still removes only the owned Run entry. This supersedes startup-off wording in the historical 0.6.1 sections below.
