...
public partial class MainWindow
{
    ...
    private void UpdatesPage()
    {
        ...
        left.Children.Add(Text("Installed version · "+BuildInfo.Version,15,"Ink",16));
        ...
        right.Children.Add(Eyebrow("Recovery"));right.Children.Add(Text("Return to a previous version",22,"Ink",14));
        ...
        if(recovery.Count==0)right.Children.Add(Text("No earlier compatible installer is saved yet. Unsigned or differently signed installers cannot be used for verified rollback.",13,"Muted",12));
        foreach(var item in recovery)right.Children.Add(Button("Reinstall "+item.Version,()=>
        {
            if(MessageBox.Show(this,"Open the verified installer for "+item.Version+"? Your settings are preserved, but features from newer versions may be unavailable.","Return to an earlier version",MessageBoxButton.OKCancel)==MessageBoxResult.OK)_updates.Launch(item.Path,item.Version);
        }));
