...
foreach(bool dark in new[]{false,true})
{
    ...
    g.Clear(ColorTranslator.FromHtml(dark?"#222532":"#E9EAF9"));
    using var ink=new SolidBrush(ColorTranslator.FromHtml(dark?"#F7F2DF":"#303550"));
    using var muted=new SolidBrush(ColorTranslator.FromHtml(dark?"#ADB4D1":"#737A95"));
    ...
    using var ground=new Pen(ColorTranslator.FromHtml(dark?"#454D70":"#CDD2EB"),2);
...
using(var sheet=new Bitmap(1120,1150))using(var g=Graphics.FromImage(sheet))
{
    ...
    {
        using var background=new SolidBrush(ColorTranslator.FromHtml(row%2==0?"#F8F7F0":"#222532"));g.FillRectangle(background,col*280,row*230,280,230);
...
using(var sheet=new Bitmap(1080,500))using(var g=Graphics.FromImage(sheet))
{
    ...
    {
        using var background=new SolidBrush(ColorTranslator.FromHtml(row==0?"#F8F7F0":"#222532"));g.FillRectangle(background,col*270,row*250,270,250);
